...
namespace KJ
{
    ...
    {
        [Header("Canvas")]
        public GameObject canvas; ///< 대화 canvas
        [Header("Raycast")]
        /* NPC 레이어 */
        ...
        /* Text 할당. */
        public TextMeshProUGUI dialogueText; ///< text 할당
        /* 대화 큐 */
        private Queue<string> _sentences; ///< 대화 Queue
        /* 한 글자씩 타이핑 */
        public float typingSpeed = 0.05f; ///< 한 글자씩 타이핑
        /* 텍스트가 한 글자씩 타이핑 중인지 확인 */
        private bool _isTyping = false; ///< 텍스트가 한 글자씩 타이핑 하는지 확인
        /* 현재 화면에 표시될 텍스트 */
        private string _currentSentence = ""; ///< 현재 화면에 표시될 텍스트
        [Header("Buttons")]
        /* 버튼들 */
        public GameObject[] choiceButtons; ///< 분기 버튼
        /* 텍스트 오브젝트  (분기 활성화시 텍스트만 사라지도록)*/
        public GameObject dialogueObjectText; ///< 분기 활성화시 텍스트만 사라짐
        /* 강화 수락 버튼 텍스트 */
        public TextMeshProUGUI enhanceText; ///< 강화 수락 버튼 텍스트
        /* 제작 수락 버튼 텍스트 */
        public TextMeshProUGUI craftText; ///< 제작 수락 버튼 텍스트
        /* 대화 UI 종료 버튼 텍스트 */
        public TextMeshProUGUI leaveText; ///< 대화 종료 버튼 텍스트
        [Header("Popup UI")]
        /* 팝업 UI */
        ...
        /* npc들 */
        [SerializeField] private List<Transform> _npcs; ///< npc들
        /* 상호작용 범위 */
        [SerializeField] private float _interactDistance = 5.0f; ///< 상호작용 범위
        /* UI 가 NPC 로부터 떨어진 거리 */
        [SerializeField] private float _offsetDistance = -15.5f; ///< UI 가 NPC 로부터 떨어진 거리
        /* 상호작용 UI */
        public GameObject interactUI; ///< 상호작용 UI
        /* 현재 상호작용UI */
        private GameObject _currentInteractUI; ///< 현재 상호작용중인 UI
        /* 가장 가까운 NPC */
        private Transform _closestNpc; ///< 가장 가까운 NPC
        /* 거리 간격 체크 (초) */
        public float checkInterval = 1f; ///< 거리 간격 체크
        [Header("Tag")]
        [Tooltip(" 코드의 복잡성을 피하기 위해 태그를 저장.")]
        private string _currentNpcTag; ///< 현재 NPC Tag (Craft, Enhance)
        [Header("Bool Active Dialogue")]
        /* 대화창 활성화 상태 */
        private bool isDialogueActive = false; ///< 대화 활성화 상태
        /* 현재 상호작용중인 Npc */
        private GameObject _currentNpc; ///< 현재 상호작용중인 NPC

        /**
         * @brief 플레이어 스폰될 때 자동으로 할당 및 대화 관련 UI 비활성화
         */
        void Start()
        {
            if (player == null)
        ...
        /**
         * @brief Raycast 를 이용하여 해당 키(F)를 누르면 상호작용 활성화
         */
        void Update()
        {
            #region Raycast
        ...
        /**
         * @brief 대화 내용을 화면에 표시
         * @param[in] npcTag npc 태그
         * @param[in] dialogueLine 대화 내용
         */
        ...
        /**
         * @brief 텍스트가 타이핑 중이면 Enter 키 누를시 문장 즉시 완성
         */
        ...
        /**
         * @brief NPC Tag 에 따라 버튼을 다르게 활성화
         */
        ...
        /**
         * @brief 가장 가까운 npc가 있는지 확인 후 상호작용 UI 활성화
         */
